...
def subdomain_discovery(
    ...
    enable_http_crawl = config.get(ENABLE_HTTP_CRAWL) or self.yaml_configuration.get(ENABLE_HTTP_CRAWL,
                                                                                     DEFAULT_ENABLE_HTTP_CRAWL)
    threads = config.get(THREADS) or self.yaml_configuration.get(THREADS, DEFAULT_THREADS)
    timeout = config.get(TIMEOUT) or self.yaml_configuration.get(TIMEOUT, DEFAULT_HTTP_TIMEOUT)
    tools = config.get(USES_TOOLS, SUBDOMAIN_SCAN_DEFAULT_TOOLS)
    default_subdomain_tools = [tool.name.lower() for tool in
                               InstalledExternalTool.objects.filter(is_default=True).filter(
                                   is_subdomain_gathering=True)]
    custom_subdomain_tools = [tool.name.lower() for tool in
                              InstalledExternalTool.objects.filter(is_default=False).filter(
                                  is_subdomain_gathering=True)]
...
def nuclei_individual_severity_module(self, cmd, severity, enable_http_crawl, should_fetch_gpt_report, ctx={},
                                      description=None):
    '''
    ...
    for line in stream_command(
        ...
        if record_exists(Vulnerability, data=vuln_data_copy, exclude_keys=object_comparison_exclude):
            logger.warning(
                f'Nuclei vulnerability of severity {severity} : {vuln_data_copy["name"]} for {subdomain_name} already exists')
            continue
...
@app.task(name='xray_scan', queue='main_scan_queue', base=RengineTask, bind=True)
def xray_scan(self, urls=[], ctx={}, description=None):
    """XSS and SQLiScan using Xray
    ...
    try:
        ...
        for item in xray_json:
            ...
            request_data = item.get('detail', {}).get('snapshot', [])[0][0]
            response_data = item.get('detail', {}).get('snapshot', [])[0][1]
            vuln_data = {
                'name': item['plugin'],
                'type': item['detail'].get('extra', {}).get('type', 'reflected'),
                'severity': 4,
                'description': item['detail'].get('extra', {}).get('param'),
...
def send_hackerone_report(vulnerability_id):
    ...
    if len(vulnerability.target_domain.h1_team_handle) != 0:
        ...
        if hackerone_query.exists():
            ...
            tpl = tpl.replace('{vulnerability_description}',
                              vulnerability.description if vulnerability.description else '')
            tpl = tpl.replace('{vulnerability_extracted_results}',
                              vulnerability.extracted_results if vulnerability.extracted_results else '')
...
def parse_nmap_results(xml_file, output_file=None):
    ...
    for host in hosts:
        ...
        if hostnames_dict:
            ...
            hostnames_list = hostnames_dict['hostname'] if isinstance(hostnames_dict['hostname'], list) else [
...
def query_whois(ip_domain, force_reload_whois=False):
	...
	"""
    if not force_reload_whois and Domain.objects.filter(name=ip_domain).exists() and Domain.objects.get(
            name=ip_domain).domain_info:
        ...
        domain_info = DottedDict(
            ...
            related_tlds=[domain['name'] for domain in
                          RelatedDomainSerializer(domain_info_db.related_tlds, many=True).data],
            related_domains=[domain['name'] for domain in
                             RelatedDomainSerializer(domain_info_db.related_domains, many=True).data],
        ...
        if domain_info_db.dns_records:
            ...
            txt_records = []
            mx_records = []
            dns_records = [{'name': dns['name'], 'type': dns['type']} for dns in
						   DomainDNSRecordSerializer(domain_info_db.dns_records, many=True).data]
        ...
        try:
            ...
            related_domains = reverse_whois(domain_info.get('registrant_email')) if domain_info.get(
                'registrant_email') else []
            for _domain in related_domains:
                related_domains_list.append(_domain['name'])
        ...
        for (field_value, count) in cl_query:
            if count > DELETE_DUPLICATES_THRESHOLD:
                ...
                for ep in eps_to_delete:
                    ...
                    if url.path in ['', '/',
                                    '/login']:  # try do not delete the original page that other pages redirect to
                        continue
...
def get_and_save_dork_results(lookup_target, results_dir, type, lookup_keywords=None, lookup_extensions=None, delay=3,
                              page_count=2, scan_history=None):
    """
